...
CREATE TABLE IF NOT EXISTS customers (
    id TEXT PRIMARY KEY, first_name TEXT NOT NULL, last_name TEXT DEFAULT '',
    phone TEXT DEFAULT '', notes TEXT DEFAULT '', created_date TEXT NOT NULL
);
CREATE TABLE IF NOT EXISTS products (
    id TEXT PRIMARY KEY, name TEXT NOT NULL, default_price REAL,
    sort_order INTEGER DEFAULT 0, active INTEGER DEFAULT 1, created_date TEXT NOT NULL
);
CREATE TABLE IF NOT EXISTS sales (
    id TEXT PRIMARY KEY, customer_id TEXT NOT NULL, customer_name TEXT DEFAULT '',
    product_id TEXT NOT NULL, product_name TEXT DEFAULT '', week TEXT NOT NULL,
    price REAL NOT NULL, created_date TEXT NOT NULL
);
CREATE TABLE IF NOT EXISTS payments (
    id TEXT PRIMARY KEY, customer_id TEXT NOT NULL, customer_name TEXT DEFAULT '',
    amount REAL NOT NULL, date TEXT DEFAULT '', notes TEXT DEFAULT '',
    item_id TEXT, item_name TEXT DEFAULT '', created_date TEXT NOT NULL
);
        ...
        async def update_item(item_id: str, payload: update_model):
            ...
            async with db_pool.connection() as conn:
                cur = await conn.execute(
                    f'UPDATE "{table_name}" SET {sc} WHERE id = %s RETURNING *',
...
async def snapshot():
    ...
    return {"customers": customers, "products": products, "sales": sales, "payments": payments, "extra_charges": extras}

SAMPLE_CUSTOMERS = [
    ("יוסף", "פרידמאן", "07700 900100"), ("שמואל", "ראזענבערג", "07700 900101"),
    ("מרדכי", "וועבער", "07700 900102"), ("חיים", "גראָסמאן", "07700 900103"),
    ("אהרן", "שווארץ", ""), ("דוד", "האָראָוויץ", "07700 900105"),
    ("נפתלי", "כ״ץ", ""), ("יצחק", "פערל", "07700 900107"),
    ("בנימין", "טייטלבוים", "07700 900108"), ("שלום", "וויינשטאק", ""),
]
SAMPLE_PRODUCTS = [
    ...
    async with db_pool.connection() as conn:
        ...
        if (await cur.fetchone())[0] == 0:
            for first, last, phone in SAMPLE_CUSTOMERS:
                ...
                ph = ",".join("%s" for _ in d)
                await conn.execute(f'INSERT INTO "customers" ({cols}) VALUES ({ph})', tuple(d.values()))
        ...
        if (await cur.fetchone())[0] == 0:
            for p in SAMPLE_PRODUCTS:
                ...
                cols = ",".join(f'"{k}"' for k in d)
                ph = ",".join("%s" for _ in d)
                await conn.execute(f'INSERT INTO "products" ({cols}) VALUES ({ph})', tuple(d.values()))
                inserted["products"] += 1
            ...
            if customers:
                ...
                for wi, week in enumerate(weeks):
                    for ai, (pid, plabel, base) in enumerate(aliyos[:8-wi%2]):
                        ...
                        sale = Sale(customer_id=cust["id"], customer_name=f"{cust.get('first_name','')} {cust.get('last_name','')}".strip(), product_id=pid, product_name=plabel, week=week, price=float(base+(ai*5)%25))
                        d = sale.model_dump()
                        cols = ",".join(f'"{k}"' for k in d)
                        ph = ",".join("%s" for _ in d)
                        await conn.execute(f'INSERT INTO "sales" ({cols}) VALUES ({ph})', tuple(d.values()))
                        inserted["sales"] += 1
                for ci, cust in enumerate(customers[:5]):
                    pay = Payment(customer_id=cust["id"], customer_name=f"{cust.get('first_name','')} {cust.get('last_name','')}".strip(), amount=float(50+ci*20), date=weeks[0], notes="צאָלונג")
                    d = pay.model_dump()
                    cols = ",".join(f'"{k}"' for k in d)
                    ph = ",".join("%s" for _ in d)
                    await conn.execute(f'INSERT INTO "payments" ({cols}) VALUES ({ph})', tuple(d.values()))
                    inserted["payments"] += 1
                extra = ExtraCharge(customer_id=customers[0]["id"], customer_name=f"{customers[0].get('first_name','')} {customers[0].get('last_name','')}".strip(), description="נדבה לחזקה", amount=50, date=weeks[1])
                d = extra.model_dump()
                cols = ",".join(f'"{k}"' for k in d)
                ph = ",".join("%s" for _ in d)
                await conn.execute(f'INSERT INTO "extra_charges" ({cols}) VALUES ({ph})', tuple(d.values()))
